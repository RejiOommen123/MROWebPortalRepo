<template>
  <div>
    <div id="AddPatientRepresentativePageBox">
      <form @submit.prevent="onSubmit" class="addPatientRepresentative-form">
        <v-row>
...
export default {
  ...
  methods: {
    onSubmit() {
      // API Call to add Patient Representative
      this.dialogLoader = true;
      this.$http.post("Master/AddPatientRepresentative", this.patientRepresentative).then(
        (response) => {
          if (response.ok == true) {
...
@media screen and (max-width: 500px) {
  #AddPatientRepresentativePageBox {
    margin: 0 0px;
  }
  h1 {
    font-size: 14px;
