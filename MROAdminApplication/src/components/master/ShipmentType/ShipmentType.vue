  ...
  <div id="ShipmentTypePageBox">
    ...
    <v-row no-gutters>
      <v-col cols="12" sm="12" md="6" >
        ...
        <span id="addRecBtn">Add Shipment Type</span>
      </v-col>
    </v-row>
    <!-- Vuetify Card with Shipment Type List Title and Search Text Box  -->
    <v-card>
      <v-card-title>
        Shipment Type List
        <v-spacer></v-spacer>
        ...
        <template v-slot:item.actions="{ item }">
          <!-- <v-tooltip top>
            <template v-slot:activator="{ on }">
              <router-link
                class="mrorouterlink"
                :to="'/ShipmentType/EditShipmentType/'+item.nShipmentTypeID"
              >
          ...
          <v-tooltip top>
            ...
            <span>Delete Shipment Type</span>
          </v-tooltip>
        </template>
      </v-data-table>
      <!-- End Shipment Type List DataTable  -->
    </v-card>

    <!-- Dialog box for Toggle Delete Shipment Type Confirmation -->
    <v-dialog v-model="dialog" max-width="360">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="red darken-1" text @click="deleteShipmentType(editedItem.nShipmentTypeID)">Yes</v-btn>
...
export default {
      ...
      headers: [
        {
          text: "Name",
          align: "start",
          value: "sShipmentTypeName",
          width: "20%",
        },
        { text: "Tooltip", value: "sFieldToolTip", width: "40%" },
        {
          text: "Normalized Name",
          value: "sNormalizedShipmentTypeName",
          width: "30%",
        },
        {
          text: "Delete",
  ...
  methods: {
    // API Call to Get all Shipment Types
    getGridData() {
      this.dialogLoader = true;
      this.$http.get("Master/GetShipmentTypes").then(
        (response) => {
    ...
    //Method to pass nShipmentTypeID & sShipmentTypeName to dialog box
    deleteItem(nShipmentTypeID, sShipmentTypeName) {
      this.editedItem.nShipmentTypeID = nShipmentTypeID;
      this.editedItem.sShipmentTypeName = sShipmentTypeName;
      this.dialog = true;
    },
    ...
    deleteShipmentType(id) {
      ...
      this.$http
        .get(
          "Master/DeleteShipmentType/sShipmentTypeID=" +
            id +
            "&sAdminUserID=" +
